...
public class DimensionalDoorBlockEntityRenderer extends BlockEntityRenderer<DimensionalDoorBlockEntity>
{
  ...
  public void render( DimensionalDoorBlockEntity entity, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay )
  {
    ...
    if( entity.getCachedState().get( DimensionalDoorBlock.HALF ) == DoubleBlockHalf.LOWER )
    {
      ...
      if( blockHitResult.getBlockPos().equals( entity.getPos() ) || blockHitResult.getBlockPos().equals( entity.getPos().up() ) )
      {
        ...
        drawCenteredTextWithRect( new TranslatableText( "text.dimensiown.dimensional_upgrades", entity.upgrades ).asString(), 0, 0xFFFFFF, false, matrices.peek().getModel(), vertexConsumers, false, 0xFFFFFF, light, 0.168F, 0.341F, 0.156F, 0.5F );
  ...
  public void drawEndTexture( DimensionalDoorBlockEntity endPortalBlockEntity, float f, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, Direction direction )
  {
    matrixStack.push();
  ...
  private void drawEndLayer( DimensionalDoorBlockEntity endPortalBlockEntity, float g, Matrix4f matrix4f, VertexConsumer vertexConsumer )
  {
  ...
  private void drawEndLayerFace( DimensionalDoorBlockEntity endPortalBlockEntity, Matrix4f matrix4f, VertexConsumer vertexConsumer, float f, float g, float h, float i, float j, float k, float l, float m, float n, float o, float p, Direction direction )
  {

    if( direction == endPortalBlockEntity.getCachedState().get( DimensionalDoorBlock.FACING ).getOpposite() )
    {
